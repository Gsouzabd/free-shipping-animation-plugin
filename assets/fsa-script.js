...
function atualizaBarraCalculo(){
    ...
    // Verifica qual regra o cep se encaixa
    var valorRegra = regraFaixaDeCep(inputValue)
    ...
    var porcentagem = (totalCarrinho / valorRegra) * 100;
...
/*
** Retorna o valor necessário de acordo com a regra de CEP
*/
function regraFaixaDeCep(inputValue) {
    console.log(inputValue);
    var formattedValue = inputValue.replace(/\D/g, '');
    
    if (/^\d{5}-\d{3}$/.test(inputValue)) {
        var numericValue = parseInt(formattedValue.replace('-', ''));
        var freteNorte = free_shipping_animation_data.frete_norte;
        var freteNordeste = free_shipping_animation_data.frete_nordeste;
        var freteSul = free_shipping_animation_data.frete_sul;
        var freteSudeste = free_shipping_animation_data.frete_sudeste;
        var freteCentroOeste = free_shipping_animation_data.frete_centro_oeste;
